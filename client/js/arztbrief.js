$(document).ready(function () {
      ...
      $('#briefoeffnenauswahlbutton').modal({
          keyboard: true,
          backdrop: "static",
          show:false,
      }).on('click', function(e){ //subscribe to show method
              ...
              $(function() {
                  $.each(data, function(i, item) {
                      if(data[i].vorlage==true){
                        ...
                        tbodyBriefe +=  "<tr onclick='openDocument(this)' id='"+ data[i]._id +"' data-dismiss='modal' style='cursor: pointer' class='patient-row'><td class='titel'>" + data[i].titel +"</td><td class='datum'>" + data[i].staticcontent.diagnose.datum +"</td><td class='lastname'>" + data[i].staticcontent.patient.nachname +"</td><td class='firstname'>" + data[i].staticcontent.patient.vorname +"</td></tr>";
                        }
                      });

                  $('#bodyVorlagenTable').html(tbodyVorlagen);
                  $('#bodyBriefeTable').html(tbodyBriefe);
                  $('#vorlagen').not('.initialized').addClass('initialized').dataTable({
                    paging: false
                  });
                  $('#briefe').not('.initialized').addClass('initialized').dataTable({
                    paging: false
                  });
              });
          },
      ...
      $('#briefspeichernbutton').modal({
        ...
        $.ajax({
          ...
          success: function(data){
              ...
              $(function() {
                  $.each(data, function(i, item) {
                        tbodyVorlagen +=  "<tr onclick='ueberspeichernBrief(this)' id='"+ data[i]._id +"' data-dismiss='modal' style='cursor: pointer' class='patient-row'><td class='titel'>" + data[i].titel +"</td><td class='datum'>" + data[i].staticcontent.diagnose.datum +"</td><td class='lastname'>" + data[i].staticcontent.patient.nachname +"</td><td class='firstname'>" + data[i].staticcontent.patient.vorname +"</td></tr>";
                      });

                  $('#bodySpeichernTable').html(tbodyVorlagen);
                  $('#briefeSpeichern').not('.initialized').addClass('initialized').dataTable({
                    paging: false
                  });
              });
          },
      ...
      for(var i = 0; i < varcontents.length; i++) {
        ...
        $.ajax({
          ...
          success: function(data){
              ...
              $('#diagnose').val(data[0].staticcontent.diagnose.diagnose);
              $('#datum').val(data[0].staticcontent.diagnose.datum);

              $('#adressatname').val(data[0].staticcontent.adressat.name);
              $('#adressataddresse').val(data[0].staticcontent.adressat.addresse);
    ...
    function ueberspeichernBrief(elem){
      var getIdFromRow = elem.id;
      ...
      var name = document.getElementById('adressatname').value;
      var addresse = document.getElementById('adressataddresse').value;

      var titel = document.getElementById('titelinput').value;
      
      var jsonSave = {"titel": titel,"vorlage": vorlage,"staticcontent":{"korrespodent":{"institut": institut,"addresse": addresse,"chefaerzte": chefaerzte,"telefon": telefon},"patient":{"nachname": nachname,"vorname": vorname,"krankenkasse": krankenkasse,"hausarzt": hausarzt},"diagnose":{"diagnose": diagnose,"datum": datum},"adressat":{"name": name,"addresse": addresse}},"varcontent": []};
